...
public class Spawner implements Runnable{
    ...
    {
        if (MyFrame.s.getTimePassed() % (MyFrame.countDowns[1] - (MyFrame.s.getTimePassed() / 15)) == 0 && (MyFrame.s.getTimePassed() != 0 && MyFrame.s.getTimePassed() != Car.timeLastSpawned)) //5 seconds to add a car is purely for testing purposes, also rewrite this without using sleep
        {
            int timesGenerated;
            Car.timeLastSpawned = MyFrame.s.getTimePassed();
            int lastSpawnedYCoord = 0;
            if (MyFrame.countDowns[1] < 6) MyFrame.countDowns[1] = 6; //spawn timer min
            timesGenerated = rand.nextInt(1, 5);
            for (int i = 0; i < timesGenerated; i++) {
                int y_axis = rand.nextInt(MyFrame.car_locations.length);
                while (y_axis == lastSpawnedYCoord) y_axis = rand.nextInt(MyFrame.car_locations.length);
                MyFrame.cars.add(new Car((int) MyFrame.size.getWidth() + 500, MyFrame.car_locations[y_axis]));
                lastSpawnedYCoord = MyFrame.car_locations[y_axis];
                Thread.sleep(300);
            }
            Car.speed += 2.5;
        }
